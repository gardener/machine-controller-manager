...
func (s *MCMServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&s.SafetyOptions.PvDetachTimeout.Duration, "machine-pv-detach-timeout", s.SafetyOptions.PvDetachTimeout.Duration, "Timeout (in duration) used for detach of PV while evicting pods")
