...
func (s *MCMServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&s.SafetyOptions.MachineDrainTimeout.Duration, "machine-drain-timeout", s.SafetyOptions.MachineDrainTimeout.Duration, "(Deprecated) No effect. Timeout (in durartion) used while draining of machine before deletion, beyond which MCM forcefully deletes machine.")
	fs.Int32Var(&s.SafetyOptions.MaxEvictRetries, "machine-max-evict-retries", controller.DefaultMaxEvictRetries, "Maximum number of times evicts would be attempted on a pod for it is forcibly deleted during draining of a machine.")
