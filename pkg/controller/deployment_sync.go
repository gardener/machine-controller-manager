...
func (dc *controller) checkPausedConditions(d *v1alpha1.MachineDeployment) error {
	...
	var err error
	d, err = dc.nodeClient.MachineDeployments().Update(d)
	return err
}
...
func (dc *controller) addHashKeyToISAndMachines(is *v1alpha1.MachineSet, machineList *v1alpha1.MachineList, collisionCount *int32) (*v1alpha1.MachineSet, error) {
	...
	updatedIS, err := UpdateISWithRetries(dc.nodeClient.MachineSets(), dc.machineSetLister, is.Namespace, is.Name,
	...
	updatedIS, err = UpdateISWithRetries(dc.nodeClient.MachineSets(), dc.machineSetLister, is.Namespace, is.Name, func(updated *v1alpha1.MachineSet) error {
...
func (dc *controller) getNewMachineSet(d *v1alpha1.MachineDeployment, isList, oldISs []*v1alpha1.MachineSet, createIfNotExisted bool) (*v1alpha1.MachineSet, error) {
	...
	if existingNewIS != nil {
		...
		if annotationsUpdated || minReadySecondsNeedsUpdate {
			...
			return dc.nodeClient.MachineSets().Update(isCopy)
		}
	...
	// the Deployment.
	alreadyExists := false
	createdIS, err := dc.nodeClient.MachineSets().Create(&newIS)
	switch {
	...
	case errors.IsAlreadyExists(err):
		alreadyExists = true
		is, isErr := dc.machineSetLister.Get(newIS.Name)
		if isErr != nil {
			return nil, isErr
		}
		...
		if !isEqual {
			...
			*d.Status.CollisionCount++
			...
			// error.
			_, dErr := dc.nodeClient.MachineDeployments().Update(d)
			if dErr == nil {
	...
	case err != nil:
		...
		if d.Spec.ProgressDeadlineSeconds != nil {
			...
			_, _ = dc.nodeClient.MachineDeployments().Update(d)
		}
...
func (dc *controller) scaleMachineSet(is *v1alpha1.MachineSet, newScale int32, deployment *v1alpha1.MachineDeployment, scalingOperation string) (bool, *v1alpha1.MachineSet, error) {
	...
	if sizeNeedsUpdate || annotationsNeedUpdate {
		...
		is, err = dc.nodeClient.MachineSets().Update(isCopy)
...
func (dc *controller) cleanupMachineDeployment(oldISs []*v1alpha1.MachineSet, deployment *v1alpha1.MachineDeployment) error {
	...
	for i := int32(0); i < diff; i++ {
		...
		if err := dc.nodeClient.MachineSets().Delete(is.Name, nil); err != nil && !errors.IsNotFound(err) {
