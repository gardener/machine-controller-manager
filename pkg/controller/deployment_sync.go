...
func calculateDeploymentStatus(allISs []*v1alpha1.MachineSet, newIS *v1alpha1.MachineSet, deployment *v1alpha1.MachineDeployment) v1alpha1.MachineDeploymentStatus {
	...
	status.LastFailedMachines = []*v1alpha1.MachineSummary{}

	for _, is := range allISs {
		if is != nil && is.Status.LastFailedMachines != nil {
			for idx := range *is.Status.LastFailedMachines {
				...
				status.LastFailedMachines = append(status.LastFailedMachines, &(*is.Status.LastFailedMachines)[idx])
			}
		}
	}
