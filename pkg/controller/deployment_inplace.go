...
func (dc *controller) syncMachineSets(ctx context.Context, oldMachineSets []*v1alpha1.MachineSet, newMachineSet *v1alpha1.MachineSet, deployment *v1alpha1.MachineDeployment) error {
	machines, err := dc.machineLister.List(labels.SelectorFromSet(newMachineSet.Spec.Selector.MatchLabels))
	if err != nil {
		return err
	}

	machinesWithUpdateSuccessfulLabel := filterMachinesWithUpdateSuccessfulLabel(machines)
	klog.V(3).Infof("machine with update successful label in new machine set %d", len(machinesWithUpdateSuccessfulLabel))

	if len(machines) > int(newMachineSet.Spec.Replicas) && len(machinesWithUpdateSuccessfulLabel) > 0 {
		...
		scaleUpBy := min(len(machinesWithUpdateSuccessfulLabel), len(machines)-int(newMachineSet.Spec.Replicas))
...
func (dc *controller) selectNumOfMachineForUpdate(ctx context.Context, allMachineSets []*v1alpha1.MachineSet, oldMachineSets []*v1alpha1.MachineSet, newMachineSet *v1alpha1.MachineSet, deployment *v1alpha1.MachineDeployment, oldISsMachinesUndergoingUpdate int32) (int32, error) {
	...
	totalReadyForUpdateCount := min(availableMachineCount-minAvailable, max(deployment.Spec.Replicas-newMachineSet.Spec.Replicas, 0))
	for _, targetIS := range oldMachineSets {
		...
		// prepare for update
		readyForUpdateCount := integer.Int32Min(targetIS.Spec.Replicas, totalReadyForUpdateCount-totalReadyForUpdate) // #nosec G115 (CWE-190) -- value already validated
		...
		numOfMachinesSelectedForUpdate, err := dc.labelMachinesToSelectedForUpdate(ctx, targetIS, readyForUpdateCount)
		if err != nil {
			return totalReadyForUpdate + numOfMachinesSelectedForUpdate, err
		}

		totalReadyForUpdate += numOfMachinesSelectedForUpdate
	}
