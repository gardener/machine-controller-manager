...
func (c *controller) reconcileClusterMachineClass(class *v1alpha1.MachineClass) error {
	...
	// 	glog.V(2).Infof("Validation of %s failed %s", MachineClassKind, validationerr.ToAggregate().Error())
	// 	return nil
	// }
	// TODO: Add this block again for generic MachineClass, it can affect certain functionalities of MachineClasses For instance if Validation failes we should not add the Finalizer on MachineClass - which would be the case now.
	...
	if class.DeletionTimestamp != nil {
		...
		glog.V(4).Infof("Cannot remove finalizer of %s because still Machine[s|Sets|Deployments] are referencing it", MachineClassKind, class.Name)
		return nil
	}
