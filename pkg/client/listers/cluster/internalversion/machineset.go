...
type MachineSetLister interface {
	...
	List(selector labels.Selector) (ret []*cluster.MachineSet, err error)
...
// List lists all MachineSets in the indexer.
func (s *machineSetLister) List(selector labels.Selector) (ret []*cluster.MachineSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*cluster.MachineSet))
	})
	return ret, err
}
...
type MachineSetNamespaceLister interface {
	...
	Get(name string) (*cluster.MachineSet, error)
...
// List lists all MachineSets in the indexer for a given namespace.
func (s machineSetNamespaceLister) List(selector labels.Selector) (ret []*cluster.MachineSet, err error) {
...
// Get retrieves the MachineSet from the indexer for a given namespace and name.
func (s machineSetNamespaceLister) Get(name string) (*cluster.MachineSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(cluster.Resource("machineset"), name)
	}
	return obj.(*cluster.MachineSet), nil
}
