...
func (c *controller) reconcileClusterMachine(ctx context.Context, machine *v1alpha1.Machine) (machineutils.RetryPeriod, error) {
	...
	// If the machine is preserved, requeue it at its PreserveExpiryTime with
	// /a delay to ensure it is processed after the preservation period ends.
	if expiryTime := machine.Status.CurrentStatus.PreserveExpiryTime; expiryTime != nil {
		if untilExpiry := time.Until(expiryTime.Time); untilExpiry > 0 && untilExpiry <= time.Duration(machineutils.LongRetry) {
...
func (c *controller) manageMachinePreservation(ctx context.Context, machine *v1alpha1.Machine) (retry machineutils.RetryPeriod, err error) {
	...
	case machineutils.PreserveMachineAnnotationValueAutoPreserved:
		if !machineutils.IsFailed(clone) || machineutils.IsMachinePreservationExpired(clone) {
