...
func (s *MCServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&s.SafetyOptions.MachineInPlaceUpdateTimeout.Duration, "machine-inplace-update-timeout", s.SafetyOptions.MachineInPlaceUpdateTimeout.Duration, "Timeout (in duration) used while in-place updating of a machine, beyond which it is declared as failed.")
