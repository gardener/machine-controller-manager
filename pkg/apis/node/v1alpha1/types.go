...
type AWSInstanceClass struct {
	...
	// +optional
	Spec AWSInstanceClassSpec `json:"spec,omitempty"`
}
...
type AWSInstanceClassList struct {
	...
	Items []AWSInstanceClass `json:"items"`
}
...
type InstanceTemplate struct {
	...
	// +optional
	Template InstanceTemplateSpec `json:"template,omitempty" protobuf:"bytes,2,opt,name=template"`
}
...
type InstanceTemplateList struct {
	...
	Items []InstanceTemplate `json:"items" protobuf:"bytes,2,rep,name=items"`
}
...
type CurrentStatus struct {
	// API group to which it belongs
	Phase				InstancePhase 			`json:"phase,omitempty" protobuf:"bytes,1,opt,name=phase,casttype=InstancePhase"`
   	// Name of instance class
...
type InstanceSetSpec struct {
	...
	InstanceClass 		ClassSpec 				`json:"instanceClass,omitempty"`
...
type InstanceSetStatus struct {
	...
	Conditions 				[]InstanceSetCondition  `json:"instanceSetCondition,inline"`

	ObservedGeneration 		int64 					`json:"observedGeneration,inline"`
}

/********************** InstanceDeployment APIs ***************/
...
type InstanceDeployment struct {
	...
	// +optional
	Spec InstanceDeploymentSpec `json:"spec,omitempty"`

	// Most recently observed status of the Deployment.
	// +optional
	Status InstanceDeploymentStatus `json:"status,omitempty"`
}
...
type InstanceDeploymentSpec struct {
	...
	Template InstanceTemplateSpec `json:"template" protobuf:"bytes,3,opt,name=template"`

	// The deployment strategy to use to replace existing pods with new ones.
	// +optional
	// +patchStrategy=retainKeys
	Strategy InstanceDeploymentStrategy `json:"strategy,omitempty" patchStrategy:"retainKeys" protobuf:"bytes,4,opt,name=strategy"`
...
const (
	...
	DefaultInstanceDeploymentUniqueLabelKey string = "pod-template-hash"
)
...
type InstanceDeploymentStrategy struct {
	...
	// +optional
	Type InstanceDeploymentStrategyType `json:"type,omitempty" protobuf:"bytes,1,opt,name=type,casttype=DeploymentStrategyType"`
	...
	// to be.
	// +optional
	RollingUpdate *RollingUpdateInstanceDeployment `json:"rollingUpdate,omitempty" protobuf:"bytes,2,opt,name=rollingUpdate"`
}
...
type InstanceDeploymentStatus struct {
	...
	// +patchMergeKey=type
	// +patchStrategy=merge
	Conditions []InstanceDeploymentCondition `json:"conditions,omitempty" patchStrategy:"merge" patchMergeKey:"type" protobuf:"bytes,6,rep,name=conditions"`
...
type InstanceDeploymentCondition struct {
	...
	Type InstanceDeploymentConditionType `json:"type" protobuf:"bytes,1,opt,name=type,casttype=InstanceDeploymentConditionType"`
	// Status of the condition, one of True, False, Unknown.
...
type InstanceDeploymentList struct {
	...
	Items []InstanceDeployment `json:"items" protobuf:"bytes,2,rep,name=items"`
}
